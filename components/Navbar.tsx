  ...
  return (
    <header
      className={`sticky top-0 z-50 transition-colors duration-300 ${
        scrolled || open
          ? "bg-offwhite/95 backdrop-blur shadow-lg shadow-navy/5"
          : "bg-offwhite/80 backdrop-blur"
      }`}
    >
      <nav className="mx-auto flex w-full max-w-7xl items-center justify-between px-6 py-4 sm:px-8 lg:px-12">
        ...
        <button
          type="button"
          className="inline-flex items-center justify-center rounded-sm p-2 text-navy-dark lg:hidden"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="overflow-hidden border-t border-navy/10 lg:hidden"
          >
                ...
                <li key={link.href}>
                  <Link
                    ...
                    className={`block py-3 font-display text-base uppercase tracking-wide transition-colors hover:text-gold ${
                      pathname === link.href ? "text-gold" : "text-charcoal/80"
                    }`}
                  >
                    {link.label}
                  </Link>
